...
update_theta <- function(theta_arg,n,x,y,d,alpha)
{
  ...
  #print(dim(theta_arg))
  #print(n)
  #print('here is the dim of n')
  #print(dim(matrix(replicate(n,theta_arg),nrow = n)))
  #print(dim(x))
  #print(n)
  #print(head(x))
  #print(theta_arg)
  
  theta_x_prod <- matrix(replicate(n,theta_arg),nrow = n) * x
  ...
  denominator <- replicate(d,denominator)
   
  dim(denominator)
  # final step, get new theta result based off update formula
  theta_arg <- theta_arg - alpha * colSums(numerator / denominator) 
...
get_final_theta <- function(theta,x,y,threshold,alpha=0.1,max_iter = 100)
{
...
theta <-rep(0,4)
x <- as.matrix(cbind(rep(1,dim(mydata)[1]),mydata[,c(2:4)]))
y <- mydata$admit
threshold <-0.0001
final_theta <- get_final_theta(theta,x,y,threshold,alpha=0.0001,max_iter = 100000)
print(final_theta)
print(coef(mylogit))
...
```{r}
# solution from: https://gist.github.com/smc77/1321542
sigmoid <- function(z) {
return(1/(1 + exp(-z)))
}
